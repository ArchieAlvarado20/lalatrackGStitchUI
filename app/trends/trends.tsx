...
import { getRideTotalsPerDay } from "@/lib/actions/trends.actions";
...
export default function TrendPage({ session }: { session: Session }) {
  ...
  const loadData = async () => {
    const result = await getRideTotalsPerDay(session.user.id);

    const formatted = result.map((d, i) => ({
  ...
  useEffect(() => {
    const init = async () => {
      await Promise.allSettled([loadData()]);
    };
